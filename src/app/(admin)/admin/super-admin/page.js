...
export default function SuperAdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] py-12 selection:bg-indigo-100 selection:text-indigo-900">
      <div className="container mx-auto px-4 max-w-7xl">
        {/* Dashboard Title & Header */}
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: '24px', marginBottom: '40px', flexWrap: 'wrap' }}>
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1">Manage administrators, control user permissions, and monitor system booking records.</p>
          </div>
        ...
        {activeTab === 'overview' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" style={{ marginBottom: '32px' }}>
              {[
                { label: 'Total Bookings', value: bookingStats.total, trend: '+12%', isPositive: true, accent: 'border-t-indigo-500', icon: '📅' },
                { label: 'Pending Approval', value: bookingStats.pending, trend: 'Needs action', isPositive: false, accent: 'border-t-amber-500', icon: '⏳' },
                { label: 'Approved', value: bookingStats.approved, trend: 'Completed', isPositive: true, accent: 'border-t-emerald-500', icon: '✅' },
                { label: 'Total Users', value: users.length, trend: '+4 this week', isPositive: true, accent: 'border-t-sky-500', icon: '👥' },
              ].map((stat, idx) => (
                <div key={idx} className={`bg-white rounded-2xl p-6 border border-slate-200/80 border-t-4 ${stat.accent} shadow-sm flex flex-col items-center text-center justify-between hover:shadow-md hover:-translate-y-1 transition-all duration-300`} style={{ minHeight: '160px' }}>
                  ...
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">{stat.label}</div>
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${stat.isPositive ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-amber-50 text-amber-700 border border-amber-100'}`}>
                    {stat.trend}
                  </span>
                </div>
              ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" style={{ marginTop: '16px' }}>
              <div className="lg:col-span-2 bg-white rounded-2xl p-8 border border-slate-200/80 shadow-sm hover:shadow-md transition-all duration-300">
              ...
              <div className="bg-white rounded-2xl p-8 border border-slate-200/80 shadow-sm flex flex-col hover:shadow-md transition-all duration-300">
                <h3 className="text-sm font-bold text-slate-900 mb-6 flex items-center gap-2">
                  <span className="w-1.5 h-3 bg-indigo-600 rounded-full"></span>
                  Status Distribution
                </h3>
